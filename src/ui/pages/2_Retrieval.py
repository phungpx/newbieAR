...
search_type = st.radio("Search mode", ["Vector (BasicRAG)", "Graph (GraphRAG)"], horizontal=True)
...
if search_type == "Vector (BasicRAG)":
    ...
    score_threshold = st.slider("Score threshold", min_value=0.0, max_value=1.0, value=0.0, step=0.01)
            ...
            if search_type == "Vector (BasicRAG)":
                ...
                if resp.status_code == 200:
                    ...
                    for i, r in enumerate(results):
                        with st.expander(f"#{i + 1} — {r['source']} (score: {r['score']:.4f})"):

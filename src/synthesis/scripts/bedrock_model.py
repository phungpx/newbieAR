...
class AmazonBedrockModel(DeepEvalBaseLLM):
    def __init__(
        ...
        **kwargs,
        ...
        if cost_per_input_token is None and "cost_per_input_token" in alias_values:
            cost_per_input_token = alias_values["cost_per_input_token"]
        if cost_per_output_token is None and "cost_per_output_token" in alias_values:
        ...
        if aws_access_key_id is not None:
            self.aws_access_key_id: Optional[SecretStr] = SecretStr(aws_access_key_id)
        else:
    ...
    async def a_generate(
        ...
        if check_if_multimodal(prompt):
            ...
            payload = self.get_invoke_request_body(prompt)

        client = await self._ensure_client()

        # Use invoke_model instead of converse
        logger.info(f"Invoking model: {self.get_model_name()} with payload: {payload}")
    ...
    def generate_payload(
        ...
        for element in multimodal_input:
            if isinstance(element, str):
                ...
                image_format = (element.mimeType or "image/jpeg").split("/")[-1].lower()

                try:
                    image_data = element.dataBase64
    ...
    def supports_structured_outputs(self) -> Union[bool, None]:
        return self.model_data.supports_structured_outputs if self.model_data else None
    ...
    async def _ensure_client(self):
        ...
        if self._client is None or self._sdk_retry_mode != use_sdk:
            ...
            retries_config = {"max_attempts": (10 if use_sdk else 1)}
            if use_sdk:
    ...
    def get_invoke_request_body(self, prompt: str) -> dict:
        """Generate request body for invoke_model API"""
        return {
            "anthropic_version": "bedrock-2023-05-31",
    ...
    def calculate_cost(self, input_tokens: int, output_tokens: int) -> float:
        ...
        # Fallback to instance-level costs
